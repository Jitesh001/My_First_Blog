...
@app.route("/")
def home():
    flash("Welcome to our first blog", "success")
    ...
    if (not str(page).isnumeric()):
        page = 1
    page = int(page)
    multi = (page-1)*int(params['no_of_posts'])
    posts = posts[multi : multi + int(params['no_of_posts'])]
    if (page == 1):
        prev = '#'
...
def contact():
    if(request.method=='POST'):
        ...
        flash("Thanks for submitting your details", 'primary')
